...
class _TutorialScreenState extends State<TutorialScreen> {
  ...
  final List<Map<String, String>> _tutorialData = [
    ...
    {
      'title': '튜토리얼 수정하기',
      'description': '튜토리얼 내용을 수정하려면:\n\n1. lib/main.dart 파일에서 _tutorialData 배열을 찾으세요\n2. 각 항목의 title, description, image를 수정하세요\n3. 이미지는 assets 폴더에 추가하고 pubspec.yaml에 등록하세요',
...
class _HomeTabState extends State<HomeTab> {
  ...
  String? _getMoodForDate(DateTime date) {
    ...
    final emotions = diary['emotions'] as Map<String, int>?;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildEmotionIndicators(diary['emotions']),
          ],
        ),
      ),
    );
    ...
    showCupertinoDialog(
      ...
      builder: (context) => CupertinoAlertDialog(
        ...
        content: Column(
          children: [
            const SizedBox(height: 16),
            ...diary['emotions'].entries.map((entry) {
              return Padding(
...
class _DiaryWriteTabState extends State<DiaryWriteTab> {
      ...
      if (mounted) {
        showCupertinoDialog(
          ...
          builder: (context) => CupertinoAlertDialog(
            ...
            content: const Text('아이쿠! 인터넷 연결을 확인해보시겠어요?'),
            actions: [
              CupertinoDialogAction(
                child: const Text('확인'),
                onPressed: () => Navigator.pop(context),
  ...
  Future<void> _processTxtFile(String filePath) async {
    try {
      ...
      // 선택된 날짜를 "일" 형태로 변환
      String selectedDay = "${_selectedDate.day}일";
      
      // Python API 호출 - 선택된 날짜 전달
      final result = await ApiService.autoDiary(
        file: file,
        searchLog: '없음',
        userPrompt: _usePrompt ? _promptController.text : null,
        usePrompt: _usePrompt,
        useDateAnalysis: true, // 날짜별 분석 활성화
        targetDate: selectedDay, // 선택된 날짜 전달
      );
      ...
      globalDiaries.add({
        ...
        'summary': _diaryResult,
        'emotions': result['emotions'] ?? {'즐거움': 50, '슬픔': 25, '분노': 25},
      });
  ...
  Future<void> _processZipFile(String filePath) async {
    try {
      ...
      // Python API 호출 (ZIP 파일도 동일한 API 사용)
      final result = await ApiService.autoDiary(
        file: file,
        searchLog: '없음',
        userPrompt: _usePrompt ? _promptController.text : null,
